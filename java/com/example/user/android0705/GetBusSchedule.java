...
public class GetBusSchedule {
    ...
    int todayWeek;
    //String SB="";
    public ArrayList AllBus=new ArrayList(); //MapActivity onMapReady呼叫到

    public void Call(TextView textView,int week) {
        tv1 = textView;
        todayWeek = week;
        String url = "http://" + id + "/BusSchedule_" + "Hsinchu" + "_Nancy.php"; //連線到此處取得資料
    ...
    private void parseJson(String s)
    {
        ...
        try {
            ...
            for (int i = 0; i < json.length(); i++) {
                //進一台新的公車就new一個，BusSchedule來記錄此公車的周一到日的時刻表
                ...
                try { RouteName= (String) ((JSONObject) json.get(i)).getJSONObject("SubRouteName").get("Zh_tw");
                        ...
                        try {
                            ...
                            for (int k= 0; k < StopTimes.length(); k++) {
                                ...
                                try {
                                    ...
                                    DepartureTimeHour = DepartureTime.substring(0,2);
                                    DepartureTimeMin =  DepartureTime.substring(DepartureTime.length()-2);
